    ...
    async def use(self, ctx, item: str = None, ammount: int = None):
        ...
        if item == None:
            ...
            if item == "1":
                ...
                if none == "[]":
                    await ctx.send("You do not have any **Common Mystery Eggs** in your inventory 3")

                else:
                    for item in items:
                        common_ammount = item[1]

                    await ctx.send(common_ammount)

                    if common_ammount < ammount:
                        await ctx.send(f"You do not have **{ammount} Commony Mystery Eggs** in your inventory 2")

                    else:
                        attempts = 0
                        max_attempts = ammount
                        number_done = 0

                        isMoneyDone = False
                        while not isMoneyDone:
                            number = int(random.randint(5, 25))
                            ...
                            if attempts == ammount:
                                ...
                                c_econ.execute(f"SELECT * FROM economy")
                                ...
                                if none == "[]":
                                    ...
                                    await ctx.send(f"You do not have {ammount} **Common Mystery Eggs** in your inventory 1 ")
                                    ...
                                    new_ammount = common_ammount - ammount

                                    c.execute(f"""UPDATE economy SET ammount = {new_ammount}
                                            WHERE user_id = '{ctx.message.author.id}'   
                                        """)
                                    ...
                                    embed: discord.Embed= discord.Embed(
                                        title="Common Mystery Egg Open",
                                        description=f"You opened **{ammount}** and got **{number_done} Dinosaur Points**"
                                    )
